...
> **このツールの正本は AICU 内部リポジトリ側にあります。**
> ここに置いているのは ComfyLTS 利用者が自分のワークフローを検査できるようにするための
> 公開版で、内部の運用知見（個別インシデント・社内環境の構成）は含めません。
> 機能追加や不具合はこのリポジトリの Issue へお願いします。
